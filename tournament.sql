...
create view match_number as (select players.ID, players.Name,
                             count(matches.Winner) as matchs
                             from players left join matches
                             on players.ID = matches.Winner
                             or players.ID = matches.Loser
                             group by matches.Winner, matches.Loser, players.ID
                             order by matchs);

create view score as (select players.ID, players.Name,
                      count(matches.Winner) as wins,
                      count(matches.Winner) as match_num
                      from players left join matches
                      on players.ID = matches.Winner
                      group by matches.Winner, players.ID
                      order by wins);

create view standings as (select score.ID, score.Name, score.wins,
                          match_number.matchs as matches
                          from score join match_number
